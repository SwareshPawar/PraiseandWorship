...
## Update (2026-03-31) - Admin Rhythm Recommender + Inline Loop Preview

### A) Admin Panel: Rhythm Recommender tab added
- Files:
    - `index.html`
    - `main1.js`
- Change:
    - Added a new Admin tab: `Rhythm Recommender`.
    - Tab can run recommendation scans for:
        - songs without `rhythmSetId` (default)
        - all songs
    - Results table includes:
        - current vs recommended rhythm set
        - recommendation score
        - per-song apply action
        - bulk apply action for checked rows
- Outcome:
    - Admin can review and selectively apply rhythm set assignments for existing DB songs from one screen.

### B) New bulk recommendation endpoint
- Files:
    - `server.js`
    - `loops/server.js`
- Change:
    - Added `GET /api/songs/bulk-rhythm-recommend` (admin only).
    - Supports `?filter=unassigned|all`.
    - Returns per-song recommendation payload generated by `recommendRhythmSetForSong`.
- Outcome:
    - UI no longer has to run recommendation logic client-side for scanning; backend provides consistent recommendation output.

### C) Inline loop player preview in recommender results
- File:
    - `main1.js`
- Change:
    - Replaced modal preview dependency with an inline loop-player row that opens directly below the clicked song.
    - Inline row includes:
        - loop player instance
        - rhythm-set dropdown (all available sets)
        - "Load in Player" button
        - "Save to Song" button
    - Selecting another rhythm set can be previewed immediately in player before saving.
- Outcome:
    - Admin can audition recommended or manually selected rhythm sets in-context without leaving the table.

### D) Direct rhythm-set selection from same screen
- File:
    - `main1.js`
- Change:
    - Dropdown is populated from `/api/rhythm-sets` (via existing `loadRhythmSets()` cache path).
    - Save action uses existing endpoint `PATCH /api/songs/:id/rhythm-set`.
    - After save, the selected set is reloaded in player for confirmation.
- Outcome:
    - Full assign-and-preview workflow is now completed inside one admin tab.

### E) Current deployment note
- Render/local (`server.js`) paths include the new bulk recommender endpoint.
- Vercel `api/songs.js` still requires separate parity wiring if this workflow must run through isolated serverless song handlers in all deployments.
